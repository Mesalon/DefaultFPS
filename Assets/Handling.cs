...
public class Handling : NetworkBehaviour {
    ...
    [SerializeField] float lookSwayFactor, lookSwayRecovery;
    [SerializeField] float moveSwayFactor, moveSwayRecovery;
    ...
    public override void FixedUpdateNetwork() {
        if (GetInput(out NetworkInputData input)) {
            if (input.buttons.IsSet(Buttons.Aim)) {
                locomotion.CurrentMoveSpeed = equippedGun.walkSpeed * equippedGun.aimingSpeedMult;
            }
            else if (input.buttons.IsSet(Buttons.Run)) {
                locomotion.CurrentMoveSpeed = equippedGun.walkSpeed * equippedGun.runningSpeedMult;
            }
            else {
                locomotion.CurrentMoveSpeed = equippedGun.walkSpeed;
            }

            if (input.buttons.WasPressed(LastInput.buttons, Buttons.Weapon1)) {
                CurrentGunType = WeaponRole.Primary;
            }

            if (input.buttons.WasPressed(LastInput.buttons, Buttons.Weapon2)) {
                CurrentGunType = WeaponRole.Secondary;
            }

            if (equippedGun.Object.IsValid) {
                if (input.buttons.WasPressed(LastInput.buttons, Buttons.Reload)) {
                    equippedGun.Reload();
                }
    ...
    public override void Render() {
        ...
        moveSwayDebt += (lastMove - new Vector3(LastInput.movement.x, kcc.FixedData.RealVelocity.y, LastInput.movement.y)) * moveSwayFactor;
        moveSwayDebt = Vector3.MoveTowards(moveSwayDebt, Vector3.zero, Time.deltaTime * 5);
        gunMoveSway = Vector3.SmoothDamp(gunMoveSway, moveSwayDebt, ref velV6, moveSwayRecovery);
        
        // Weapon Recoil 
        Vector2 appliedPosRecoil = currentPosRecoil * equippedGun.Recoil.posSpeed * Time.deltaTime;
        Vector2 appliedRotRecoil = currentRotRecoil * equippedGun.Recoil.rotSpeed * Time.deltaTime;
        Vector2 appliedCamRecoil = currentCamRecoil * equippedGun.Recoil.camSpeed * Time.deltaTime;
        ...
        gunRecoilPose.position = Vector3.Slerp(gunRecoilPose.position, Vector3.zero, equippedGun.Recoil.posRecovery * Time.deltaTime);
        gunRecoilPose.rotation = Quaternion.Slerp(gunRecoilPose.rotation, Quaternion.identity, equippedGun.Recoil.rotRecovery * Time.deltaTime);
        ...
        equippedGun.transform.localRotation = gunHandlePose.rotation * gunRecoilPose.rotation * gunBobPose.rotation * gunLookSway * Quaternion.Euler(-gunMoveSway.z, gunMoveSway.x, 0);
